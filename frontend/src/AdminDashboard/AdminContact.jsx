...
const AdminContact = () => {
  ...
  useEffect(() => {
    axios.get("https://coffeehouse-4yii.onrender.comapi/admin/messages")
      .then((res) => setMessages(res.data))
      .catch((err) => console.error(err));

    axios.get("https://coffeehouse-4yii.onrender.comapi/admin/branches")
      .then((res) => setBranches(res.data.branch ? res.data.branch.branches : []))
      .catch((err) => console.error(err));
  }, []);

  const handleReply = (id) => {
    axios.patch(`https://coffeehouse-4yii.onrender.comapi/admin/messages/${id}/reply`, reply)
  ...
  const handleDeleteMessage = (id) => {
    axios.delete(`https://coffeehouse-4yii.onrender.comapi/admin/messages/${id}`)
      .then(() => setMessages(messages.filter((msg) => msg._id !== id)))
      .catch((err) => console.error(err));
  };

  const handleBlockSender = (id, email) => {
    axios.patch(`https://coffeehouse-4yii.onrender.comapi/admin/messages/${id}/block`)
  ...
  const handleBranchSubmit = () => {
    axios.post("https://coffeehouse-4yii.onrender.comapi/admin/branches/add", branchForm)
