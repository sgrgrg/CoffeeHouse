...
const EditBanner = () => {
  ...
  useEffect(() => {
    axios.get("backend-production-402e.up.railway.app/api/banner")
      .then((res) => {
        const data = res.data || {};
        setBanner({
          title: data.title || "",
        ...
        setPreviewImage(data.image ? `backend-production-402e.up.railway.app/${data.image}` : "");
      })
  ...
  const handleSubmit = (e) => {
    ...
    axios.put(`backend-production-402e.up.railway.app/api/banner/${banner._id}`, formData, {
      headers: {
