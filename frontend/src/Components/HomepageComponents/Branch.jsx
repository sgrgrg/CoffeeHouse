...
const Branch = () => {
  ...
  useEffect(() => {
    ...
    axios.get('https://coffeehouse-1.onrender.com/api/admin/branches')
      .then((response) => {
            ...
            featuredBranches.map((branch, index) => (
              <div key={branch._id} className="col-6 col-md-3 mb-4" data-aos="zoom-in" data-aos-delay={(index + 1) * 100}>
                <div className="branch">
                  <img
                    src={`https://coffeehouse-1.onrender.com/${branch.image}`} 
                    alt={branch.location} 
                    className="img-fluid branch-img"
                    onClick={() => openModal(`https://coffeehouse-1.onrender.com/${branch.image}`)} // Open modal with the clicked image
                  />
